...
#endif
{
	...
	printf("Display Stream Compression (DSC) reference model version 1.31a\n");
	...
	while ((strlen(infname)>0) || !feof(list_fp))
	{
		...
		if ((function == 1) || (function == 0))
		{
			...
			{
				if (dsc_codec.slice_height >= 8)
					firstLineBpgOfs = 12 + ((int)(0.09 * MIN(34, dsc_codec.slice_height - 8)));
				else
					firstLineBpgOfs = 2 * (dsc_codec.slice_height - 1);
			}
...
#define SW_PPS_MODIFY

#ifdef SW_PPS_MODIFY
			if (dsc_codec.slice_width == dsc_codec.pic_width / 2) {
				dsc_codec.initial_dec_delay = 736;
				dsc_codec.nfl_bpg_offset = 23;
				dsc_codec.scale_increment_interval = 34286;
				//dsc_codec.first_line_bpg_ofs = 12;
			}
			else if (dsc_codec.slice_width == dsc_codec.pic_width){
				dsc_codec.initial_dec_delay = 1216;
				dsc_codec.nfl_bpg_offset = 23;
				dsc_codec.scale_increment_interval = 42286;
				//dsc_codec.first_line_bpg_ofs = 12;
			}
#endif
		...
		int SW_EXYNOS_MODIFY_OPT = 1;

		if (SW_EXYNOS_MODIFY_OPT) {
			dsc_codec.initial_dec_delay = 1216;
			dsc_codec.scale_increment_interval = 34286;
			dsc_codec.scale_decrement_interval = 13;
			dsc_codec.nfl_bpg_offset = 23;
			dsc_codec.slice_bpg_offset = 14;
		}
		slicecount = 0;
